...
Code này không production-ready. Nó gặp 3 lỗi chí mạng: lộ bí mật (hardcode key), không thể triển khai Cloud (cứng localhost/port) và thiếu khả năng tự phục hồi (không có health check). Đây thuần túy là code chạy thử trên máy cá nhân.
...
|---------|-------|----------|---------------------|
| Config | Hardcode | Env vars | Bảo mật, linh hoạt giữa các môi trường |
| Health check |Không có  |/health, /ready  | Giúp nền tảng Cloud tự monitor và restart khi lỗi |
| Logging | print() | JSON | Dễ truy vết lỗi, tránh log nhầm thông tin nhạy cảm |
...
Stage 1 (Builder): Cài đặt các công cụ build hệ thống (như gcc) và tải toàn bộ thư viện Python (dependencies) cần thiết.

Stage 2 (Runtime): Tạo môi trường chạy thực tế bằng cách chỉ copy các thư viện đã build và mã nguồn từ Stage 1 sang. Thiết lập bảo mật với non-root user và cấu hình Healthcheck.

Tại sao image nhỏ hơn: Vì Stage 2 loại bỏ hoàn toàn các công cụ build nặng nề, file rác và cache phát sinh ở Stage 1. Image cuối cùng chỉ chứa những thành phần tối thiểu để ứng dụng có thể vận hành.
...
- API key được check ở đâu?\
    Được check tại hàm **`verify_api_key`** . Hàm này được đưa vào endpoint `/ask` thông qua cơ chế `Depends` của FastAPI để kiểm tra header `X-API-Key`.
- Điều gì xảy ra nếu sai key?\
    Hệ thống sẽ ném ra một ngoại lệ `HTTPException` với mã lỗi **`403 Forbidden`** và thông báo "Invalid API key.". (Nếu thiếu key hoàn toàn, lỗi trả về sẽ là 
- Làm sao rotate key?\
    Thay đổi giá trị của biến môi trường **`AGENT_API_KEY`** (trong file `.env` hoặc cấu hình Docker) và khởi động lại (restart) ứng dụng để nhận key mới.
