...
# Redis setup (Exercise 4.4)
try:
    r = redis.Redis(host='localhost', port=6379, db=0, socket_timeout=1)
    # Ping to check connection
    r.ping()
    REDIS_AVAILABLE = True
    logger.info("Redis connected for Cost Guard")
except Exception:
    REDIS_AVAILABLE = False
    r = None
    logger.warning("Redis not available. Using in-memory fallback for Cost Guard.")
...
# ──────────────────────────────────────
# Exercise 4.4: Redis-based Budget Check
# ──────────────────────────────────────

def check_budget_redis(user_id: str, estimated_cost: float) -> bool:
    """
    Exercise 4.4 Implementation (Redis pattern).
    Kiểm tra budget từ Redis theo tháng.
    """
    if not REDIS_AVAILABLE:
        # Fallback to in-memory check if Redis is down
        logger.debug("Redis down, bypassing Redis budget check")
        return True
